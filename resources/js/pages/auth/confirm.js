...
$(function () {
    ...
    $(document).on('click', '#formConfirmPassword #btnSubmit', function (e) {
        e.preventDefault();
        const btnSubmit = this;
        $(btnSubmit).attr('data-indicator', 'on').prop('disabled', true);
        const formData = new FormData($('#formConfirmPassword')[0]);
        $.ajax({
            type: "POST",
            url: `${origin}/password/confirm`,
            data: formData,
            processData: false,
            contentType: false,
            dataType: "json",
            success: function (response) {
                ...
                toastSuccess('Password has been confirmed');
                setTimeout(() => {
